...
namespace cgp
{
	...
	rotation_transform rotation_transform::between_vector(vec3 const& e, vec3 const& e_target)
	{
		assert_cgp(is_equal(norm(e), 1.0f), "Vector e="+str(e)+" must have a unit norm ("+str(norm(e))+")");
	...
	rotation_transform rotation_transform::between_vector(vec3 const& e1, vec3 const& e2, vec3 const& e1_target, vec3 const& e2_target)
	{
