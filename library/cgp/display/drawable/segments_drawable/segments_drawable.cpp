...
namespace cgp
{
	...
	segments_drawable& segments_drawable::initialize(buffer<vec3> const& position, std::string const& object_name, GLuint shader_arg)
	{
		// Sanity check and warning on command line if initialize is called on some segments_drawable already associated to data
		if (vbo_position != 0)
			warning_cgp("Warning try to initialize segments_drawable [" + name + "] with existing data","");
	...
	segments_drawable& segments_drawable::update(buffer<vec3> const& new_position)
	{
		opengl_update_gl_subbuffer_data(vbo_position, new_position);
		return *this;
	}

	segments_drawable& segments_drawable::clear()
	{
