...
struct grid_3D
{
    ...
    * \note: the size of the 3D-buffer must satisfy arg.size = size_1 * size_2 * size_3 */
    static grid_3D<T> from_vector(buffer<T> const& arg, int size_1, int size_2, int size_3);
...
template <typename T>
grid_3D<T> grid_3D<T>::from_vector(buffer<T> const& arg, int size_1, int size_2, int size_3)
{
