	...
	{
		{
			...
			assert_cgp_no_msg(!is_equal(a, cgp::buffer_stack<int, 3>{ 1,2,3 }));
			assert_cgp_no_msg(!is_equal(a, cgp::buffer_stack<int, 5>{ 1, 2, 3, 4, 5 }));
		}

		{
			cgp::buffer_stack<float,3> a = { 1.1f, 2.2f, 3.1f };
			assert_cgp_no_msg(is_equal(a, { 1.1f,2.2f,3.1f }));
			assert_cgp_no_msg(!is_equal(a, cgp::buffer_stack2<float>{ 1.1f,2.2f }));
			assert_cgp_no_msg(!is_equal(a, { 1.1f,2.21f,3.1f }));
		}

		// access size 2
		{
			cgp::buffer_stack<int, 2> a = { 1,2 };
		...
		// access size 3
		{
			cgp::buffer_stack<int,3> a = { 1,2,3 };
			a[0] = -1;
			a[1] = 5;
			a[2] = a[1] + 1;
		...
		{
			cgp::buffer_stack<int,3> const a = { 1,2,3 };
		...
		{
			cgp::buffer_stack<cgp::buffer_stack<int,2>,3> a = { cgp::buffer_stack<int,2>{1,2}, {2,3}, {-1,1} };
		...
		{
			cgp::buffer_stack<float, 16> b; assert_cgp_no_msg(cgp::is_equal(type_str(b), "buffer_stack<float,16>"));
			cgp::buffer_stack<cgp::buffer_stack<int,5>,6> c; assert_cgp_no_msg(cgp::is_equal(type_str(c), "buffer_stack<buffer_stack<int,5>,6>"));
		}

		{
			cgp::buffer_stack<int,3> a = { 4,5,6 };
		...
		{
			cgp::buffer_stack<int,3> a = { 7,5,-1 };
			assert_cgp_no_msg(cgp::size_in_memory(a)==3*sizeof(int));

			cgp::buffer_stack<cgp::buffer_stack<int,2>,3> b = { cgp::buffer_stack<int,2>{1,2},{2,3}, {-1,-1} };
			assert_cgp_no_msg(cgp::size_in_memory(b) == 6 * sizeof(int));
		}

		{
			cgp::buffer_stack<std::string, 2> a = { "hello","world" };
