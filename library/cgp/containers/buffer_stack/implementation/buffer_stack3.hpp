...
namespace cgp
{
    ...
    template <typename T> int buffer_stack<T, 3>::size() const
    {
        return 3;
    }
    ...
    template <typename T> T const& buffer_stack<T, 3>::at(int index) const
    {
        check_index_bounds(index, *this);
        return (&x)[index];
    }
    template <typename T> T& buffer_stack<T, 3>::at(int index)
    {
        check_index_bounds(index, *this);
        return (&x)[index];
    }
    ...
    template <int idx, typename T> T const& get(buffer_stack<T, 3> const& data)
    {
        static_assert(idx < 3, "Incorrect element indexing");
        return (&data.x)[idx];
    }
    template <int idx, typename T> T& get(buffer_stack<T, 3>& data)
    {
        static_assert(idx < 3, "Incorrect element indexing");
        return (&data.x)[idx];
    }
