...
struct buffer
{
    ...
    buffer(int size);                  // Buffer with a given size 
    ...
    * Linear interpolation between p1 and p2 along N variable */
    ...
    T const& at(int index) const; // Internal call to std::vector.at
    T & at(int index);            // Internal call to std::vector.at
...
template <typename T> int size_in_memory(buffer<T> const& v);
...
template <typename T>
buffer<T>& buffer<T>::resize(int size)
{
...
template <typename T>
buffer<T>& buffer<T>::resize_clear(int size)
{
    clear();
    resize(size);
    return *this;
...
template <typename T>
void check_index_bounds(int index, buffer<T> const& data)
{
#ifndef cgp_NO_DEBUG
...
template <typename T>
T const& buffer<T>::at(int index) const
{
    return data.at(index);
}

template <typename T>
T & buffer<T>::at(int index)
{
    return data.at(index);
}

template <typename T>
T const& buffer<T>::at_unsafe(int index) const
{
    return data[index];
}

template <typename T>
T& buffer<T>::at_unsafe(int index)
{
    return data[index];
}
...
{
    int const N = a.size();
    assert_cgp(N>0, "Cannot compute average on empty buffer");
...
{
    int const N = v.size();
    assert_cgp(N>0, "Cannot get max on empty buffer");
...
template <typename T>
buffer<T> buffer<T>::linespace(T const& p1, T const& p2, int N)
{
    buffer<T> buf; 
    buf.resize(N);
