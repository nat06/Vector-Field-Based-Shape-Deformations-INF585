...
void integrate(mesh & shape, buffer<vec3> const& position_before_deformation, vec3 const& picked_position, deformer_parameters_structure const& deformer_parameters, grid_3D<vec3>&velocity, grid_3D<vec3> const& grid, sphere_tool_structure const& sphere_tool, vec3 const& previous_tool_pos, vec3 const& tr, vec3 & constant_vel, vec3 const& picked_normal, enum constant_velocity_direction velocity_dir_type, buffer<buffer<int>> one_ring) {
