...
void scene_structure::mouse_move(cgp::inputs_interaction_parameters const& inputs)
{
	...
	if (inputs.keyboard.shift)
	{
		...
		if (inputs.mouse.click.left && picking.active && gui.constant_velocity_parameters.type == direction_mouse_movement) {
			...
			if (!previous_interactive_deformation) previous_mouse_position = inputs.mouse.position.current;//this just started to move !

			vec2 const tr_2D = inputs.mouse.position.current - inputs.mouse.position.previous; // previous mouse position (previous position since no update)
			...
			vec2 const tr_2D_2 = inputs.mouse.position.current - inputs.mouse.position.previous; // translation in screen coordinates -> should not be done like this !
...
void scene_structure::mouse_click(cgp::inputs_interaction_parameters const& inputs) {
	...
	if (inputs.keyboard.shift && gui.constant_velocity_parameters.type != direction_mouse_movement)//we dont use this when we move along the mouse velocity
		//the deformation is only applied when shift + click
	{
		...
		integrate(deforming_shape.shape, deforming_shape.position_saved, picking.position, velocity, grid, sphere_tool, previous_tool_pos, previous_tool_pos, constant_vel, picking.normal, deforming_shape.one_ring, gui.constant_velocity_parameters);
...
void scene_structure::initialize()
{
	...
	deforming_shape.update_normal();
	update_velocity_field(velocity, grid, sphere_tool, previous_tool_pos, previous_tool_pos, vec3(0, 0, 0), constant_vel, gui.constant_velocity_parameters);
...
void scene_structure::display()
{
	...
	{ //might want to do this only whn no deformation
		...
		update_velocity_field(velocity, grid, sphere_tool, previous_tool_pos, previous_tool_pos, constant_vel, picking.normal, gui.constant_velocity_parameters);//TO FIX!
		update_velocity_visual(velocity_visual, velocity_grid_data, velocity, grid, scale); //HERE ???
		if (timer.event) {
...
void scene_structure::display_arrow(){
	...
	//vec3 const& p0, vec3 const& p1, float cylinder_radius, float cone_length_scale, float cone_radius_scale, int N

	arrow_visual.clear();
	if (norm(constant_vel) >= 0.5) {
